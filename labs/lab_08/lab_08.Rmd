...
PG_Calls <- PG_Calls |>
  group_by(month) |>
  summarize(total_calls = n()/1397*100) |>
  arrange(desc(total_calls)) |>
  mutate(pct_total = total_calls)
...
PG_Calls |>
  ggplot() +
  geom_bar(aes(x=reorder(month,pct_total),weight=pct_total)) +
  coord_flip() +
  labs(
    title = 'One in Ten Overdose Calls in Prince George County\'s County happens in December',
    x = 'month',
...
```{r}
#Find the total number of calls
...
```{r}
PG_Calls_zipcodes <- PG_Calls |> group_by(zipcode) |> summarize(total_calls = n())
...
ggplot(pg_calls_with_income, aes(x=estimate, y=total_calls)) + 
  geom_point()
...
ggplot(pg_calls_with_income, aes(x=estimate, y=total_calls)) + 
  ...
  geom_text(aes(x= estimate, y= total_calls, label = zipcode), check_overlap = TRUE)
