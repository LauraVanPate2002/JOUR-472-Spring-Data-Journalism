...
**A1.** <https://datawrapper.dwcdn.net/6YP6K/1/>
...
mutate(pct_cox = cox/(cox + moore + lashar + wallace + harding + sefcik + write_ins.x),
...
**A2.** <https://datawrapper.dwcdn.net/TrIbQ/1/>
...
md_votes_by_type |>
  mutate(
    pct_total = total/(total +year + early + election_day + mail.x),
    pct_year = year/(year + early + election_day + mail +total.y)
  )
...
**A3.** <https://datawrapper.dwcdn.net/CHBG4/1/>

```{r}
Montgomery_County_Calls <- read_csv('data/montgomery_2022_overdoses.csv') |>
  clean_names()
  mutate(
    pct_zip = zip/(zip + incident_id +crime_reports + start_time + end_time + priority + intial_type + close_type + address + city + state + longitude + latitude + police_district_number + beat + pra + call_time_call_route + calltime_dispatch + calltime_arrive + calltime_cleared + call_route_dispatch + dispatch_arrive + arrive_cleared + disposition_desc.x),
    pct_calls = calls/(calls + zip + incident_id +crime_reports + start_time + end_time + priority + intial_type + close_type + address + city + state + longitude + latitude + police_district_number + beat + pra + call_time_call_route + calltime_dispatch + calltime_arrive + calltime_cleared + call_route_dispatch + dispatch_arrive + arrive_cleared + disposition_desc.y))
  write_csv (Montgomery_County_Calls, 'data/montgomery_2022_overdoses.csv')
```
