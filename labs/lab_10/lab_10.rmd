...
md_statewide_county <- md_statewide_county |>
  ...
  mutate(pct_moore = (moore/total_gov) * 100,
         pct_brown = (brown/brown/total_ag) * 100, 
...
mutate(outage_customers + total_customers) |>
  
mutate(pct_outage_customers = (outage_customers-total_customers/total_customers
        ) *100,
        pct_total_customers = (total_customers-outage_coustomers/total_customers) *100)
```

```{r}
#Merge geometries with outage data
county_outages_march2023_geometry <- county_outages_march2023 |>
  
  inner_join(county_outages_march2023, by = 'geoid')
...
ggplot() +
  geom_sf(data = county_outages_march2023, aes(fill=pct_difference))
```
...
ev_2020_2023
mutate(march_2023 - july_2020/july_2020) *100
```
...
ev_2020_2023_geometry <- ev_2020_2023 |>
  ...
  inner_join(ev_2020_2023, by = 'geoid')
...
#Map out the change
ggplot() +
  geom_sf(data = ev_2020_2023, aes(fill=pct_difference))
...
mutate(estimate + GEOID/GEOID) *100
...
ggplot() +
  geom_sf(data = md_county_population, aes(fill=pct_difference))
