...
A1. Carroll has the most with 448.
...
A2. August is the worst month for all three counties. 314 Cecil County.
...
maryland_zcta |> filter(zcta5n == '21217') 
  
maryland_zcta |> summarize(avg_pct_rent = mean(pct_rent))
```
...
A4: Something I want to know is how many calls in Allegany, Carroll, Cecil, and Baltimore happened in the month of April? That data didn't work and I think a reason why is because of override and because I wasn't combing correctly.

```{r}
combined_all_together <- bind_rows(list(combined_911,baltimore_911_with_demographics)) |>
  mutate(month = month(date)) |>
  group_by(date,month) |>
  summarize(date = n())
```
