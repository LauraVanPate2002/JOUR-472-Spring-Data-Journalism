...
```{r}
emergency_calls <- read_csv('~/GitHub/JOUR-472-Spring-Data-Journalism/labs/lab_05/tabula-Baltimore County; Carey, Samantha log OD-1.csv', col_names = FALSE) |>
  clean_names() |>
...
A1. **July 14 and October 4 23. No. 366**

```{r}
calls_by_date <- emergency_calls |>
  group_by(date) |>
...
A2. Friday, Saturday, and Sunday are the highest with Monday and Sunday having the highest percentage.
...
A3. **The location with the most calls was 115 Susquehanna W. AV and 6424 Windsor Mill Rd. who both had 13. Both of them are in unsafe areas with the Maryland State Police Department warning people to stay away from those areas. There isn't really anything I worry about, but there always is dirty data that can throw things off.**

```{r}
locations <- emergency_calls |>
  group_by(location) |>
...
A4. **A story I'd be interested in is how often to fentaly overdoses happen or at least recorded and why do they happen more often in certain areas.**
