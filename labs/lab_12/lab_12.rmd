...
stop_words_plus <- stop_words |>
  ...
  unique_words_top_10_plus <- unique_words |> anti_join(stop_words_plus) |>
  group_by(word) |>
  tally(sort = TRUE) |>
  mutate(percentage = (n/sum(n))*100) |>
  top_n(10)
...
**A1. The leading word is failing, and I think the top ten words describe clients and how the lawyer company failed their trust and consent.**
...
atty_trigram <- md_attorney_sanctions |>
...
**A2. The top trigram is attorney trust account, appearing 343 times and 5.7% of the story. I think it means that clients are suppose to be able to trust their attorney and in this situation they weren't.**

------------------------------------------------------------------------

**Q3.** Let's drop back down to more traditional text analysis - take the top trigram from Q2 and write code to see how many times it occurs in the text column in each fiscal year. What do you think the answer produced by your code suggests? What else could you do to try and clarify the most important reasons attorneys get sanctioned? **Look at Filter Pre_Lab. 1, 2, 3**

```{r}
attorney_trust_account <- md_attorney_sanctions |>
  filter(fiscal_year == 2023) |>
  filter(text == attorney_trust_account)
...
**A3. I'm not exactly sure how many times it did since my code didn't work, but since it appeared 343 times, it should appear around 12 times each physical year. I think a group by and summarize could work to show why an attorney can get sanctioned, since we could group by text and summarize by words such as disbarred or reprimanded.**
