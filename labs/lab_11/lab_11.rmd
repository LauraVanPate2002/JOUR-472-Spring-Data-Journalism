...
grants_url <- "https://grants.maryland.gov/Pages/StateGrants.aspx"
  ...
  # make a list of organizations that have the most grants
...
opd_releases <- opd_results |> html_elements('a.wixui-rich-text__text') |>
  html_text() |> as_tibble() |>
  separate(value, c('date', 'title'), sep= '.') |>
  drop_na(title) |> mutate(title = str_squish(title), date = mdy(date))

opd_results <- opd_url |> read_html() |> html_elements('a.wixui-rich-text__text') |>
  html_text() |> 
  as_tibble() |>
  separate(value, c('date', 'title'), sep= '.') |>
  drop_na(title) |> mutate(title = str_squish(title), date = mdy(date))
...
```{r}
press_release_url <- "https://www.cardin.senate.gov/?post_type=press-releases."

press_release_results <- press_release_url |>
  read_html()

press_release_releases <- press_release_results |> html_elements('a.wixui-rich-text__text') |>
  html_text() |> as_tibble() |>
  separate(value, c('date', 'title'), sep= '.') |>
  drop_na(title) |> mutate(title = str_squish(title), date = mdy(date)) |>
 head(html_attr(html_nodes(, "a"), "href"))

press_release_results <- press_release_url |> read_html() |> html_elements('a.wixui-rich-text__text') |>
  html_text() |> 
  as_tibble() |>
  separate(value, c('date', 'title'), sep= '.') |>
  drop_na(title) |> mutate(title = str_squish(title), date = mdy(date)) |>
