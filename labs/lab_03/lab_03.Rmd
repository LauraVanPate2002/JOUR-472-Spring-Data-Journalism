...
md_payments <- read_csv("data/State_of_Maryland_Payments_Data__FY2008_to_FY2024.csv") |> 
  clean_names()
...
pg_schools <- md_payments |>
  filter(str_detect(vendor_name, 'PRINCE GEORGES PUBLIC SCHOOL') |
...
A4: Something that I noticed about the data was that in the fiscal year 2009 the total amount was over 14 million, however, in 2010, the total amount decreased to around half of that being a little over 7 million. The total amount from 2011-2013 went up somewhere in between 8 million and 9 million, but in 2014 the total amount went down to 6 million. The total amount did get back up to 7 million in 2019, but is still only half of what it was in 2009.
