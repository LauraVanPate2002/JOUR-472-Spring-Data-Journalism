...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer: Data Vendor is character. Amount field is double. Date is character.**
...
**I decided to come up with this equation and I thought it made the most sense. I figured that md_grants_loans should be first because zipcodes are under the data for md_grants_loans. Following that I decided to put zip codes because we're looking for which zip code is the highest number. After that I figured summarizing the zip codes would be the best idea because we learned that in previous labs and pre-labs, and that way we could get the zip codes in order. Finally I thought that arranging in descending order would help because that way we could find the zip code with that got the most amount of money.**

```{r}
md_grants_loans |>
  zip_codes |>
  summarize(zipcodes) |>
  arrange(desc(zipcodes))
